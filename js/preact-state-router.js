// preact-state-router.js - Single file router with state management
// Implementation following the detailed plan from koder/plan-preact-state-router.md
...
async function loadRoute(routeConfig, params) {
  ...
  try {
    ...
    routerState.value = { ...routerState.value, error: { type: 'load_failed', message: error.message, originalError: error, recoverable: true } };
    return {};
  } finally {
...
function importState(data) {
  try {
    ...
    routerState.value = { ...routerState.value, data: { ...routerState.value.data, ...parsed } };
  } catch (error) {
...
function createStateRouter(config) {
  ...
  if (match) {
    ...
    routerState.value = {
      ...
      data: persistedState,
      isLoading: false,
      error: { type: 'route_not_found', message: `Route not found: ${path}`, recoverable: true }
    };
  }
  ...
  return {
    ...
    setState: (newData) => {
      routerState.value = { ...routerState.value, data: { ...routerState.value.data, ...newData } };
    },
    exportState,
    importState,
    routes,
